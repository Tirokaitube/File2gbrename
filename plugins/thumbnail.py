...
async def viewthumb(client, message):    
    ...
    if thumb:
       await client.send_photo(chat_id=message.chat.id, photo=thumb)
        ...
        await message.reply_text("**You Don't Have Any Thumbnail @Anime_Lumino ❌**") 

@Client.on_message(filters.private & filters.command(['del_thumb', 'delthumb']))
async def removethumb(client, message):
    await jishubotz.set_thumbnail(message.from_user.id, file_id=None)
    await message.reply_text("**Thumbnail Deleted Successfully @Anime_Lumino 🗑️**")

@Client.on_message(filters.private & filters.photo)
async def addthumbs(client, message):
    mkn = await message.reply_text("Please Wait ...")
    await jishubotz.set_thumbnail(message.from_user.id, file_id=message.photo.file_id)                
    await mkn.edit("**Thumbnail Saved Successfully @Anime_Lumino ✅️**")
